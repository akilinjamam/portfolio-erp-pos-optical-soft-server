...
const createPayrollService = async (data) => {
    ...
    if (payrollData.length === 0) {
        ...
        const firstData = {
            ...
            due: dueInString,
            netSalary: findEmployee.basicSalary,
            totalSalary: findEmployee.basicSalary,
            ...remainingData,
        }
        const result = await Payroll.create(firstData);
...
const createPayrollBonusService = async (data) => {
    const { incentive, overtime, employeeName, paymentMethod, transectionId, ...remaining } = data;

    const payrollData = await Payroll.find({ employeeName: employeeName });

    if (payrollData?.length === 0) {

        const findEmployee = await Employee.findOne({ _id: employeeName });

        const totalSalary = Number(incentive) + Number(overtime) + Number(findEmployee?.basicSalary)
    ...
    const { paid, totalSalary, totalPaid } = lastPayroll;

    const newTotalSalary = (Number(incentive) + Number(overtime) + Number(totalSalary))?.toString();
